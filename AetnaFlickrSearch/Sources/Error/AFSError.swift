...
extension AFSError: LocalizedError {
...
private struct Log {
    static var general: OSLog = {
        let bundleID = Bundle.main.bundleIdentifier ?? "cbd.AetnaFlickrSearch"
...
func AFSLog(fullname: String = #function, file: String = #file, line: Int = #line, andReport: Bool = false, _ items: Any?...) {
...
func createError(_ description: String) -> AFSError {
    return AFSError.customError(description: description)
}

/// Create a custom error from the provided response code
/// - Parameter errorCode: a server response code
func createError(_ errorCode: Int) -> AFSError {
    switch errorCode {
    case 400:
